...
_.mixin({
  ...
  compactObject: function(to_clean) {
    _.map(to_clean, function(value, key, to_clean) {
      if (value === undefined) delete to_clean[key];
    });
    return to_clean;
  }
});
  ...
  var req = https.request(options, function(res) {
    ...
    res.on("end", function() {
      if (res.statusCode !== 200) {
        ...
        return callback(
          new BitstampError("Bitstamp error " + res.statusCode, message)
        );
      }
      try {
...
Bitstamp.prototype._post = function(
  market,
  action,
  callback,
  args,
  ...
  if (!this.key || !this.secret || !this.client_id)
    return callback(
      new Error(
        "Must provide key, secret and client ID to make this API request."
      )
    );

  if (legacy_endpoint) var path = "/api/" + action + "/";
  else {
    if (market) var path = "/api/v2/" + action + "/" + market + "/";
    else var path = "/api/v2/" + action + "/";
  }
...
Bitstamp.prototype.order_book = function(market, group, callback) {
  ...
  var options;
  if (typeof limit === "object") options = group;
  else options = { group: group };
...
Bitstamp.prototype.buy = function(
  market,
  amount,
  price,
  limit_price,
...
Bitstamp.prototype.sell = function(
  market,
  amount,
  price,
  limit_price,
...
Bitstamp.prototype.bitcoin_withdrawal = function(
  amount,
  address,
  instant,
  callback
...
Bitstamp.prototype.xrp_withdrawal = function(
  amount,
  address,
  destination_tag,
  callback
...
Bitstamp.prototype.withdrawal = function(
  amount,
  account_currency,
  name,
  IBAN,
...
Bitstamp.prototype.ripple_withdrawal = function(
  amount,
  address,
  currency,
  callback
...
Bitstamp.prototype.transfer_to_main = function(
  amount,
  currency,
  subAccount,
  callback
...
Bitstamp.prototype.transfer_from_main = function(
  amount,
  currency,
  subAccount,
  callback
...
Bitstamp.prototype.new_liquidation_address = function(
  liquidationCurrency,
  callback
) {
